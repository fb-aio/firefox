(() => {
  ...
  function runInBackground2(fnPath, params) {
  ...
  (() => {
    ...
    function run() {
      ...
      function isFocused() {
        ...
        return new Promise((resolve) => {
          ...
          function onReceiveMsg(e) {
            if (e.data?.type === checkFocusMsg + "result" && e.data?.uuid === uuid) {
      ...
      async function getIdleState() {
        ...
        let tabs = await runInBackground3("browserApi.tabs.query", [
          {
            active: true,
            audible: true,
            url: location.href
        ...
        let state = await runInBackground3("browserApi.idle.queryState", [t]);
        if (state != "active")
          return {
            isIdle: true,
      ...
      async function saveTimer() {
        ...
        focusTimerValue = 0;
        await runInBackground("browserApi.storage.local.set", [{ web_timer }]);
      }
      async function getTodayTimer() {
        let host = location.hostname || location.pathname?.split("/")?.at(-1) || location.href;
        let today = getToday();
        let result = await runInBackground("browserApi.storage.local.get", [
          ["web_timer"]
        ]);
      ...
      async function makeTitle(revert) {
        ...
        let showTimeOnTitle = result?.[key];
        if (showTimeOnTitle) {
