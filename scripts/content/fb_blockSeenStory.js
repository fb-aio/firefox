(() => {
  ...
  // scripts/content/fb_blockSeenStory.js
  (async () => {
    ...
    hookXHR2({
      onBeforeSend: ({ method, url, async, user, password }, dataSend) => {
        if (method === "POST" && dataSend?.toString?.()?.includes?.("storiesUpdateSeenStateMutation")) {
          notify2({ msg: "\u{1F440} FB AIO: facebook story seen BLOCKED" });
          return null;
        }
      }
    });
